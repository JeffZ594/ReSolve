/**
 * @file LinSolverIterativeBAGMRES.cpp
 * @author Kasia Swirydowicz (kasia.swirydowicz@pnnl.gov)
 * @brief Implementation of LinSolverIterativeBAGMRES class
 *
 */
#include <iostream>
#include <cassert>
...
#include "LinSolverIterativeBAGMRES.hpp"

namespace ReSolve
{
  using out = io::Logger;

  LinSolverIterativeBAGMRES::LinSolverIterativeBAGMRES(MatrixHandler* matrix_handler,
  ...
  LinSolverIterativeBAGMRES::LinSolverIterativeBAGMRES(index_type     restart,
                                                     real_type      tol,
                                                     index_type     maxit,
  ...
  LinSolverIterativeBAGMRES::~LinSolverIterativeBAGMRES()
  {
    if (is_solver_set_) {
      freeSolverData();
    }
   ...
   */
  int LinSolverIterativeBAGMRES::setup(matrix::Sparse* A, matrix::Sparse* B)
  {
  ...
  int  LinSolverIterativeBAGMRES::solve(vector_type* rhs, vector_type* x)
  {
  ...
  int  LinSolverIterativeBAGMRES::setupPreconditioner(std::string type, LinSolverDirect* LU_solver)
  {
    if (type != "LU") {
  ...
  int  LinSolverIterativeBAGMRES::resetMatrix(matrix::Sparse* new_matrix)
  {
    A_ = new_matrix;
   ...
   */
  int LinSolverIterativeBAGMRES::setOrthogonalization(GramSchmidt* gs)
  {
    GS_ = gs;
    return 0;
  }
   ...
   */
  int LinSolverIterativeBAGMRES::setRestart(index_type restart)
  {
   ...
   */
  int LinSolverIterativeBAGMRES::setConvergenceCondition(index_type conv_cond)
  {
    conv_cond_ = conv_cond;
    return 0;
  }

  index_type  LinSolverIterativeBAGMRES::getRestart() const
  {
    return restart_;
  }

  index_type  LinSolverIterativeBAGMRES::getConvCond() const
  {
    return conv_cond_;
  }
  ...
  index_type LinSolverIterativeBAGMRES::getCliParamInt(const std::string id) const
  {
    switch (getParamId(id))
    {
      case MAXIT:
  ...
  real_type LinSolverIterativeBAGMRES::getCliParamReal(const std::string id) const
  {
    switch (getParamId(id))
    {
      case TOL:
  ...
  int LinSolverIterativeBAGMRES::printCliParam(const std::string id) const
  {
    switch (getParamId(id))
    {
    case TOL:
  ...
  int LinSolverIterativeBAGMRES::allocateSolverData()
  {
  ...
  int LinSolverIterativeBAGMRES::freeSolverData()
  {
    delete [] h_H_ ;
    delete [] h_c_ ;
    delete [] h_s_ ;
  ...
  void LinSolverIterativeBAGMRES::precV(vector_type* rhs, vector_type* x)
  {
    LU_solver_->solve(rhs, x);
  }

  void LinSolverIterativeBAGMRES::setMemorySpace()
  {
  ...
  void LinSolverIterativeBAGMRES::initParamList()
  {
